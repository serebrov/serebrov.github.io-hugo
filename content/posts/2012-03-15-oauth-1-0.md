...
* A) Consumer requests Request Token
  * call `get_request_token` from Service Provider, send
    ...
    * `oauth_signature`
    * ...
    * here `oauth_signature` is the signature of the request created using Consumer Secret, simplified example:
    * `$signature = md5($request_text . $consumer_secret)`
    * both sides (Consumer and Service Provider) know consumer_secret and are able to perform this operation, so Service Provider can check whether signature is valid
...
* E) Consumer requests Access Token
  * call `get_access_token`, send
    ...
    * ...
    * `oauth_verifier`
  * here `oauth_signature` is the signature of the request created using request token secret from B
  * note, that on step A Consumer uses his Consumer Secret to sign the request and here he uses request token secret
...
[introduction to oauth](http://thinkvitamin.com/code/introduction-to-oauth/)
