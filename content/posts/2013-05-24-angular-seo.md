...
The simple way to make this content SEO-friendly is to pre-render data on the server and then allow angular to do its job on the client.
<!-- more -->
...
Note that the solution above works for simple cases but it may not work well for
complex pages or if there are many pages.  Actually what we do is duplicate the
content - we provide static content on the server side and then angular
requests content dynamically and overrides static data.
...
[BromBone](http://www.brombone.com/).  It will do the complex part of the job for
...
[AngularJS and SEO - approach based on PhantomJS](http://www.yearofmoo.com/2012/11/angularjs-and-seo.html) and [github repository](https://github.com/steeve/angular-seo)
